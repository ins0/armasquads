...
class SquadImageService
{
    ...
    public function convertFromTGA($sourceImage)
    {
        Try {
            ...
            $command = 'cd '.escapeshellcmd(escapeshellarg(dirname($sourceImage))).' && wine /bitools/ImageToPAA.exe ' . escapeshellcmd(escapeshellarg(basename($tgaPath))) . ' ' . escapeshellcmd(escapeshellarg(basename($tgaPath, '.tga') . '.paa'));
    ...
    public function convert($sourceImage)
    {
        Try {
            // convert to paa
            $command = 'cd '.escapeshellcmd(escapeshellarg(dirname($sourceImage))).' && wine /bitools/ImageToPAA.exe ' . escapeshellcmd(escapeshellarg(basename($sourceImage))) . ' ' . escapeshellcmd(escapeshellarg(basename($sourceImage, '.png') . '.paa'));
            exec($command, $response);
    ...
    public function saveLogo( $logoSpecs )
    {
        Try {
            ...
            $image->writeImage('png32:' . $saveLogoPath);
            $image->destroy();
            $image->clear();
